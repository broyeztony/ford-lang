...
function Sol_VariableDeclarationStatement(node, metadata) {
  ...
  const {varName, varType, genericType, initializer} = node.declarations[0]

  if (genericType === 'HASHMAP') {
    throw new Error('Hashmaps cannot be created dynamically, you have to assign them from a state variable.')
  }
  ...
  if (initializer && initializer.hasOwnProperty('value') || (initializer && initializer.arguments && initializer.arguments.length > 0)) {
    let solVarValue;
    switch (varType) {
      case 'u8'   :
      case 'u16'  :
